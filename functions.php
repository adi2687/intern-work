...
function strtoinr($price)
{
    ...
    if (fmod($price, 1) == 0) {
        // If no decimal part, use the number without decimals
        $formattedPrice = $inr->formatCurrency((int) $price, $currency);
    } else {
        // If there is a decimal part, keep it as float
        $formattedPrice = $inr->formatCurrency($price, $currency);
    }

    // Remove trailing ".00" if present (NumberFormatter may add it)
