...
class Auth
{
    ...
    // register without verification (for oder creation)
       public function registerWithoutVerify($name,$phone, $role='user', $status = "pending")
    {
        // Sanitize fields
        DB::connect();
        ...
        $this->userID = md5(md5($this->phone.$this->name).md5(time().uniqid()));
        ...
        if ($validate['error']) {
            ...
            if ($this->error) {
                ...
                return ['error' => $this->error, 'userID' => $this->userID, 'message' => 'Registration successful'];
            }
        }
    ...
    public function register($name,$phone, $orderId, $role, $status = "pending")
    {
        // Sanitize fields
        DB::connect();
        $this->name = trim(DB::sanitize($name));
    ...
    // Send OTP
    public function sendOTP($name,$phone,$role){
        ...
        $register = $this->register($name,$phone,$result['orderId'],$role);   
          if($register){
            $this->error = false;
            $this->errorMsgs['sendOTP'] = '';
